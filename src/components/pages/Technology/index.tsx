...
const Technology = () => {
  ...
  React.useEffect(() => {
    const  getTechnology = async () => {
      try {
      const {data} = await axios.get<TechnologyItems[]>(`https://63404624e44b83bc73cd3e47.mockapi.io/Technology?id=${technologyId}`)
      setItems(data)
      } catch (err) {
        setIsError(true)
      }
          ...
          isError ? <div className='error'>connection error</div>
            : <>
              ...
              <MediaQuery minWidth={1300}>
                {items.map(obj => <TechnologyImage key={obj.name} name={obj.name} image={obj.images.portrait}/>)}
              </MediaQuery>
              <MediaQuery maxWidth={1299}>
                {items.map(obj => <TechnologyImage key={obj.name} name={obj.name} image={obj.images.landscape}/>)}
              </MediaQuery>
              </div>
            </>
        }
